...
def show_profile(request,person_id):
    ...
    if person_id in [person.id for person in StudentInfoModel.objects.all()]:
        if StudentInfoModel.objects.get(pk=person_id).company_id_id==request.user.company_id_id:
            return redirect('https://dormoni.com/person_panel/student/'+StudentInfoModel.objects.get(pk=person_id).id)
        else:
            return render(request, '404.html')
    elif person_id in [person.id for person in PersonalInfoModel.objects.all()]:
        if PersonalInfoModel.objects.get(pk=person_id).company_id_id==request.user.company_id_id:
            return redirect('https://dormoni.com/person_panel/personal/' + PersonalInfoModel.objects.get(pk=person_id).id)
        else:
            return render(request, '404.html')
    elif person_id in [person.id for person in ParentInfoModel.objects.all()]:
        if ParentInfoModel.objects.get(pk=person_id).company_id_id==request.user.company_id_id:
            return redirect('https://dormoni.com/person_panel/parent/' + ParentInfoModel.objects.get(pk=person_id).id)
        else:
            return render(request, '404.html')
    elif person_id in [person.tcn for person in PersonIDInfoModel.objects.all()]:
        if PersonIDInfoModel.objects.get(pk=person_id).company_id_id==request.user.company_id_id:
            return redirect('https://dormoni.com/person_panel/person_id/' + PersonIDInfoModel.objects.get(pk=person_id).tcn)
        else:
            return render(request, '404.html')
    else:
        if person_id in [person.id for person in User.objects.all()] and request.user.is_superuser==False:
            person = User.objects.get(pk=person_id)
            return redirect('https://dormoni.com/user_panel/user/' + str(person.id))
        else:
            return render(request,'404.html')
